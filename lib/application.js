 ...
 * `emvc` constructor.
 *
 * A default `emvc` singleton is exported via the module.  Applications
 ...
 *
 * emvc provides support for content negotiation, allowing a single route
 ...
 *
 * By default, emvc looks for template files using a convention of
 ...
 *
 * To facilitate mapping models to controllers, emvc introspects models
 ...
 * should be registered with emvc to provide the necessary introspection
 * logic.
 *
 * @param {Module} store
 * @api public
 ...
 * a specific controller and action in this emvc appliction.  This is
 ...
 *
 *     this.use(emvc.invoke());
 *
 * If you are mounting multiple emvc applications in a single server, a
 ...
 *
 * Controller's are auto-required by emvc.  The value exported by the
 ...
 *
 * emvc uses an MVC-style router, where routes map from a URL pattern to
...
/**
 * Boot `emvc` application.
 *
 * emvc builds on Express, providing a set of conventions for how to
 * organize code and resources on the file system as well as an MVC architecture
 * for structuring code.
 *
 * When booting a emvc application, the file system conventions are used
