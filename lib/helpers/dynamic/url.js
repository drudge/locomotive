 ...
 * in which case emvc will introspect the record to build the
 * corresponding URL.
 *
 ...
 *
 * When taking advantage of model awareness, be sure to configure emvc
...
function urlFor(req, res) {
  ...
  return function(options) {
    ...
    options.controller = router.util.controllerize(options.controller) || req._emvc.controller;
    options.action = router.util.functionize(options.action) || req._emvc.action;
