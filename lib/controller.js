...
Controller.prototype.params = function(name, defaultValue) {
  ...
  } else if (typeof this.__req.query === 'object' && this.__req.query.hasOwnProperty(arguments[0])) {
    return this.__req.query[arguments[0]];
  } else {
...
Controller.prototype.render = function(template, options, fn) {
  ...
  // delete option keys consumed by emvc
 ...
 *     })
 *
 * By default emvc passes an `Error` with a `.status` of 406 to
...
/**
 * Invoked when controller is initialized by emvc.
 *
 * This function is called immediately after emvc instantiates the
...
Controller.prototype._invoke = function(action) {
  ...
  // Assign emvc properties to request.  These properties are used by
  // helper functions, such as urlFor(), to return results based on the context
  // of the request and response.
  this.__req._emvc = {};
  this.__req._emvc.app = this.__app;
  this.__req._emvc.controller = this.__id;
  this.__req._emvc.action = this.__action;
