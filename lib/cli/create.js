...
/**
 * Create emvc application at `path`.
 *
 * @param {String} path
 * @api private
 */
exports = module.exports = function create(path) {
  console.log('creating emvc application at : ' + path);
...
var pagesController = [
    'var emvc = require(\'emvc\')'
  , '  , Controller = emvc.Controller;'
  , ''
...
var mainTemplate = [
  ...
  , '  <body>'
  , '    <h1><%= title %></h1>'
  , '    <p>Welcome aboard! Visit <a href="http://emvcjs.org/">emvcjs.org</a> for details.</p>'
  , '  </body>'
  , '</html>'
].join(eol);
...
var routesTemplate = [
  ...
  , '// Guide on [routing](http://emvcjs.org/guide/routing.html) for additional'
  , '// information.'
...
var allEnvironments = [
  ...
  , '  // of emvc.'
  , '  if (this.version !== require(\'emvc\').version) {'
  , '    console.warn(\'version mismatch between local (%s) and global (%s) emvc module\', require(\'emvc\').version, this.version);'
  , '  }'
  , '}'
  , ''
].join(eol);
...
var viewsInitializer = [
  ...
  , ''
  , '  // Override default template extension.  By default, emvc finds'
...
var serverJS = [
  ...
  , 'app.phase(emvc.boot.httpServer(3000, \'0.0.0.0\'));'
  , ''
